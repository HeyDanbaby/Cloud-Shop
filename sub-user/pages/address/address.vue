<template>
	<view class="padding-lr padding-bottom-big margin-bottom ft-main bg-drak">
		<use-navbar title="我的地址"></use-navbar>
		...
		<view class="pos-f btn-container padding-tb bg-drak">
			<view class="dflex-b border-radius-big safe-area-inset-bottom-plus">
				<view v-if="is_mp" class="tac padding-tb-sm flex1 bg-gray" @click="importAddr">{{ platform_name }}导入</view>
	...
	export default {
		...
		methods: {
			...
			addAddr(type, options) {
				...
				uni.navigateTo({
					url: `/sub-user/pages/address/address-edit?type=${type}&id=${options._id}`
				});
			},
			// 删除收货人
			removeAddr(options) {
