<template>
	<view class="content bg-drak ft-main">
		...
		<view class="row dflex padding-left">
			...
			<view class="dflex-b flex1">
				...
				<!-- #ifdef MP-WEIXIN || MP-BAIDU || MP-ALIPAY || APP -->
				<view class="pos-r">
					<view class="vertical-line" style="right: calc(100% + 20rpx);"></view>
					<view class="dflex" @click="choiceMap">
						<text class="ft-dark-3">地图选择</text>
						<view class="iconfont icondizhi- input_t padding-lr-sm padding-tb-16"></view>
					</view>
				</view>
				<!-- #endif -->
		...
		<view class="use-area margin-lr-sm">
			<view class="tit">详细地址</view>
			<textarea class="bg-drak w-full border-radius-sm margin-top-sm padding" style="min-height: 70px;" v-model="addrData.addr_detail" maxlength="120" auto-height placeholder="请输入详细地址"></textarea>
		</view>
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.type === 'edit') {
				...
				if (this.islogin && this.member.member_mobile) {
					this.addrData.mobile = this.member.member_mobile;
				}
			}
		...
		methods: {
			...
			choiceMap(res) {
				...
				this.choiceMapAddr(res);
				// #endif
				// #ifdef MP-ALIPAY
				this.choiceAlipayMapAddr(res);
				// #endif
			},
			// 选择地图地址
						...
						uni.chooseLocation({
							success: res => {
								...
								if (res && res.name) {
									...
									// #ifdef APP
									_this.addrData.addr_detail = res.address.replace(__addr.addr.replace(/-/g, ''), '');
									// #endif
								}
							},
							fail(err) {
