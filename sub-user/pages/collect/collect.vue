<template>
	<view class="bg-drak" :class="[empty ? '' : 'padding-tb']">
		<use-navbar title="我的收藏"></use-navbar>
		...
		<view v-else class="padding-lr" v-for="(item, index) in datas" :key="index">
			<view class="product border-radius padding margin-bottom-sm bg-main" style="padding-bottom: 15rpx;">
				...
				<view class="margin-left-sm pos-r w-full">
					<text class="clamp-2" @click="togoods(item)">{{ item.goods.name }} {{ item.goods.name_pw }}</text>
					<view class="pos-a dflex-b price-box w-full">
						<text class="price padding-tb-sm" @click="togoods(item)">{{ item.goods.price / 100 }}</text>
...
<script>
	...
	export default {
		...
		onShareAppMessage: function(ops) {
			...
			let goods = this.datas.find(x => x.goods_id == ops.target.id);
			let share_img = '';
			if (goods && goods.id > 0) {
			...
			return {
				imageUrl: share_img,
				bgImgUrl: share_img,
				title: `来自 [${this.member.user_name}] 的收藏夹`,
				path: `/sub-goods/pages/detail?id=${ops.target.id}&mid=${mid}`,
				content: this.$config.appname || '用云电商',
				desc: this.$config.appname || '用云电商',
		...
		methods: {
			loadData() {
				...
				db.collection(_collect, goodsTemp)
					...
					.field(
						'_id, visit_cnt, last_modify_time, goods.price, goods.name, goods.name_pw, goods._id as goods_id, goods.img, goods.state'
					)
			...
			deleteCollect(id) {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							this.$db[_collect].where('create_uid == $env.uid').remove(id).then(res => {
								if (res.code === 200) {
									this.loadData();
								}
							})
			...
			clear() {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							this.$db[_collect].where('create_uid == $env.uid').remove().then(res => {
								if (res.code === 200) {
									this.datas = [];
									return;
								}
