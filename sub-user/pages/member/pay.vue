		...
		methods: {
			...
			confirm() {
				...
				this.prePayment((preRes) => {
					...
					this.$func.usemall.call('vip/pay', obj).then(res => {
						...
						if (res.code === 200) {
							if (res.datas) {
								...
								pay_datas = {
									...
									getOrderStatus: (res) => {
										...
										return new Promise((resolve, reject) => {
											...
											this.$func.usemall.call("vip/paystate", {
												pay_id: out_order_no
											}).then(res => {
								...
								// 调用支付
								this.topayment(pay_datas, res.datas.pay_id);
							} 
							return;
						}
			...
			topayment(pay_datas, pay_id) {
				...
				uni.requestPayment({
					...
					success: (pres) => {
						...
						uni.redirectTo({
							url: `/sub-user/pages/member/member?pay_id=${this.pay_id}`
						});
						return;
					},
					fail: (err) => {
			...
			loadQRCode(){
				...
				let obj = {
					pay_way: this.pay_way,
					pay_original: this.pay_original || '微信支付',
					pay_trade_type: this.pay_trade_type || 'NATIVE',
				};
				uni.showLoading({
					title: '请求中'
				})
