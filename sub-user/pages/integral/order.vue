					...
					<view class="padding-lr margin-bottom-sm" :class="index === 0 ? 'padding-top-sm' : ''"
						...
						<view class="order-item padding bg-main border-radius">
							<view @click="todetail(item.order)">
								...
								<view class="goods-area" :class="{ 'margin-top': goodsIndex > 0 }"
									v-for="(goodsItem, goodsIndex) in item.order_detail" :key="goodsIndex">
									<image :src="goodsItem.goods_sku_img || goodsItem.goods_img" mode="aspectFill"></image>
									<view class="right flex1">
										...
										<view class="margin-top-sm">
											<text class="price-jf ft-main fs-sm">{{ goodsItem.goods_integral}}</text>
										</view>
									</view>
								</view>
								...
								<view class="dflex-e">
									<text class="fs-xs margin-right-xs">实付款</text>
									<text class="price-jf ft-main">{{ item.order.order_actural_paid }}</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			async loadData(source = 'add', loading) {
				...
				await this.$func.usemall.call('integral/orderList', this.reqdata).then(res => {
					if (res.code === 200) {
			...
			todetail(order) {
				uni.navigateTo({
					url: `/sub-user/pages/integral/order-detail?order_id=${order.order_id}`
				});
			},
			// 删除订单
			delOrder(item) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							_this.$func.usemall.call('integral/orderDeleted', {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							_this.$func.usemall.call('integral/orderCancel', {
								order_id: item.order.order_id,
								state: '已取消'
							}).then(res => {
								if (res.code === 200) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							_this.$func.usemall.call('integral/orderReceived', {
								order_id: item.order.order_id,
								state: '待评价'
							}).then(res => {
								_this.loadData('tab_change', 1);
			...
			toevaluate(item) {
				uni.navigateTo({
					url: `/sub-user/pages/integral/order-evaluate?id=${item.order.order_id}`
				});
			}
		}
	};
