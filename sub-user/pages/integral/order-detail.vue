		...
		<view class="total-area padding margin-lr margin-tb-sm bg-main border-radius">
			<view class="ft-dark">
				<view class="item dflex-b">
					...
					<text class="">{{ order_data.order_total_money }}</text>
				</view>
				<view class="item dflex-b">
					<text>运费</text>
					<text class="">￥0</text>
			...
			<view class="item dflex-b">
				<text>实付款</text>
				<text class="price-jf">{{ order_data.order_actural_paid }}</text>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			async loadData() {
				let _this = this;
				await this.$func.usemall.call('integral/orderDetail', {
			...
			evaluate() {
				uni.navigateTo({
					url: `/sub-user/pages/integral/order-evaluate?id=${this.order_id}`
				});
			},
		},
	}
