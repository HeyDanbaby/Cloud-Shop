<template>
	<view class="container bg-main pos-r">
		<use-navbar title="注册"></use-navbar>
		...
		<view class="padding-xl dflex-c dflex-flow-c">
			...
			<view v-if="agreementDatas && agreementDatas.length > 0" class="dflex margin-top" @click="toAgreement">
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			this.$db['usemall-sys-agreement'].where({ type: '注册' }).tolist().then(res => {
				if (res.code == 200) {
					this.agreementDatas = res.datas;
					this.agreement = !(this.agreementDatas && this.agreementDatas.length > 0);
		...
		methods: {
			...
			register() {
				...
				// #ifdef H5 || MP-360 || APP-PLUS || MP-WEIXIN
